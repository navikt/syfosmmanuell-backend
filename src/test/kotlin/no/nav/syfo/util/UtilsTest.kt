...
class UtilsTest : FunSpec({
    context("Log NAV Epost from token when no access") {
        test("should not throw excpetion when has claim preferred_username") {
            val token = generateJWT("2", "clientId", Claim("preferred_username", "firstname.lastname@nav.no"))!!
            shouldNotThrow<Exception> { logNAVEpostFromTokenWhenNoAccessToSecureLogs(token, "/thispath") }
        }
